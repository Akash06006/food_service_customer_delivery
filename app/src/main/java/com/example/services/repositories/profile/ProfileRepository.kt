package com.example.services.repositories.profile
...
class ProfileRepository {
    ...
    fun updateDates(jsonObject: JsonObject?): MutableLiveData<CommonModel> {
        if (jsonObject != null) {
            val mApiService = ApiService<JsonObject>()
            mApiService.get(
                object : ApiResponse<JsonObject> {
                    override fun onResponse(mResponse: Response<JsonObject>) {
                        ...
                        data3!!.postValue(logoutResponse)
                ...
                }, ApiClient.getApiInterface().updateDates(jsonObject)
            )
