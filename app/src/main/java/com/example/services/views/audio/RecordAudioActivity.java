package com.example.services.views.audio;
...
//
    ...
    private void playSound(final MediaPlayer player) {
        if (player != null) {
            if (timerStart == false) {
                player.start();
                timerStart = true;
                            ...
                            public void run() {
                                if (player != null && player.isPlaying()) {
                                    tvTime.post(new Runnable() {
                                        @Override
                                        public void run() {
                                            tvTime.setText("00:" + player.getCurrentPosition() / 1000);
                                        }
                                    });
                                } else {
                                    btnPlay.setBackgroundResource(R.drawable.aar_ic_play);
        ...
        if (startRecording == false) {
            if (checkPermission()) {
                ...
                try {
                    ...
                    countDownTimer = new CountDownTimer(20900, 1000) {
